...
final case class MapEventWidget[
  ...
  DownEvent,
  A, B
](body : Widget[[W] =>> Update[W, A], Draw, Place, Recomposition, DownEvent], f : A => B) extends Widget[[W] =>> Update[W, B], Draw, Place, Recomposition, DownEvent]:

  override def handleDownEvent(pathToParent: Path, event: DownEvent): Update[Place[Widget[[W] =>> Update[W, B], Draw, Place, Recomposition, DownEvent]], B] =
    body.handleDownEvent(pathToParent, event).map(_.map(copy(_))).mapSecond(f)
  end handleDownEvent

  override def asUnplaced: Place[Widget[[W] =>> Update[W, B], Draw, Place, Recomposition, DownEvent]] =
    body.asUnplaced.map(copy(_))
  end asUnplaced

  override def mergeWithState(pathToParent: Path, oldState: Map[String, StateTree[Recomposition]]): Place[Widget[[W] =>> Update[W, B], Draw, Place, Recomposition, DownEvent]] =

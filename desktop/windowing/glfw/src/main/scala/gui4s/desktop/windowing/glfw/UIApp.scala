...
trait UIApp extends IOApp:
  ...
  final def runResourced : Resource[IO, ExitCode] =
    ...
    for
      ...
      (width, height) <- glfw.getFramebufferSize(window).eval

      surface <- SkijaSurface.create(width, height).evalOn(MainThread)
        ...
        draw =>
          surface.drawFrame(sur =>
            for
              ...
              _ <- glfw.pollEvents
            yield (),
            FunctionK.lift([T] => syncIO => syncIO.to)

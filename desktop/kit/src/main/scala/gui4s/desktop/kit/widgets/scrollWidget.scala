...
final case class RichInnerEvent(scrollChange: Point2d[Float], childSize: Rect[Float], ownSize: Rect[Float])
...
def scrollWidget[
  ...
  scrollEventSource(glfw, window, eventBus).map(
    scrollWidget(
      _,
      animation
    )
...
def scrollWidget(
  ...
  new ScrollWidget:
    override def apply[Event](
      ...
      transitiveStatefulWidget[Float, Event, RichInnerEvent](
        name = stateName,
        initialState = 0f,
        eventHandler = (state, events) =>
          events.foldM(state) {
            case (currentState, RichInnerEvent(scrollEvent, childSize, ownSize)) =>
              updateScrollAccordingToTheEvent(eventSource, ownSize, childSize, scrollEvent, currentState, direction, scrollSpeed)
          },
        body = count =>
          animation(
...
def updateScrollAccordingToTheEvent[Event](
                                            eventSource: ScrollEventSource,
                                            ownSize : Rect[Float],
                                            childSize : Rect[Float],
                                            scrollEvent : Point2d[Float],
                                            currentState : Float,
                                            direction : Axis,
                                            scrollSpeed : Float
) : Update[Event, Float] =
  val minScroll = (ownSize.along(direction) - childSize.along(direction)).min(0f) / scrollSpeed
  val nextState = currentState + scrollEvent.along(direction)
  ...
  (
    if leftover != 0f then
      eventSource.emitLeftovers(scrollEvent.mapAlong(direction, _ => leftover))
    else
      ().pure[Update[Event, *]]
    ).as(clampedState)

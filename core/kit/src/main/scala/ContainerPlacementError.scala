...
final case class ContainerPlacementError[Error](
  ...
  def map[NewError](f : Error => NewError) : ContainerPlacementError[NewError] =
    ContainerPlacementError(f(withSpaceBetweenStrategy), f(withSpaceAroundStrategy), f(withCenterStrategy), f(withEndStrategy))

object ContainerPlacementError:
  val English: ContainerPlacementError[Throwable] = ContainerPlacementError(
  ...
  val Russian : ContainerPlacementError[Throwable] = ContainerPlacementError(

...
public class ShaderSPIRVUtils {
    ...
    public static SPIRV compileShaderFile(String shaderFile, ShaderKind shaderKind) throws Exception {
        var resource = getSystemClassLoader().getResource(shaderFile);
        if (resource == null)
            throw new Exception("Couldn't find sharer file " + shaderFile);
        return compileShaderAbsoluteFile(resource.toExternalForm(), shaderKind);
    }

...
final class HighLevelApiImpl[
  ...
  SystemEvent >: TaskFinished
    ...
    using val liftReaction : LiftEventReaction[Update, WidgetTaskIn[Any]],
          val linearLayoutLibrary: LayoutLibrary[Place, [A] =>> PlacedWidget[Update, Draw, Place, A, SystemEvent], LayoutPlacementMeta[MU]]
)(
  val drawApi : SimpleDrawApi[MU, Draw],
  val placement : [Event] => (Axis, List[Place[PlacedWidget[Update, Draw, Place, Event, SystemEvent]]], MainAxisPlacementStrategy[MU], AdditionalAxisPlacementStrategy) => Place[List[(PlacedWidget[Update, Draw, Place, Event, SystemEvent], LayoutPlacementMeta[MU])]]
) extends HighLevelApi with LabelApi[TextStyle] with StatefulApi with LayoutApi[MU]:
  override type WidgetTask[+T] = WidgetTaskIn[T]
  override type Widget[+Event] = Place[PlacedWidget[Update, Draw, Place, Event, SystemEvent]]
  ...
  override def label(text: String, style : TextStyle): Widget[Nothing] =
    library.label(using textDraw, summon, library.drawOnlyWidget)(text, style)
  end label
  
  given statefulDraw : StatefulDraw[Draw] with
    override def drawStateful[T](name : String, state : State[?, T, ?], childTree: PlacedWidget[?, Draw, ?, ?, ?]): Draw = childTree.draw
  end statefulDraw
  ...
  override def stateful[T: Equiv, ParentEvent, ChildEvent](
    ...
    library.stateful[Update, Draw, Place, T, ParentEvent, ChildEvent, SystemEvent, WidgetTaskIn](name, initialState, eventHandler, renderState andThen addTaskResultCatcher[ChildEvent](using checkEvent)(name), checkState)
  end stateful
  ...
  private def linearLayout[Event](
                                  ...
                                  )(using Widget[Event] =:= Place[PlacedWidget[Update, Draw, Place, Event, SystemEvent]]): Widget[Event] =
    linearLayoutLibrary.layout(children, placement[Event](axis, _, mainAxisStrategy, additionalAxisStrategy))
  end linearLayout
  
  
  private def addTaskResultCatcher[T](using RichTypeChecker[T])(name: String)(initial: Place[PlacedWidget[Update, Draw, Place, T, SystemEvent]]) : Place[PlacedWidget[Update, Draw, Place, T, SystemEvent]] =

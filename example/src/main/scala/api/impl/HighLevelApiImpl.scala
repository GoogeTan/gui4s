...
final class HighLevelApiImpl[
  ...
  SystemEvent >: TaskFinished
    ...
    using val liftReaction : LiftEventReaction[Update, WidgetTaskIn[Any]],
          val linearLayoutLibrary: LayoutLibrary[Place, [A] =>> Widget[Update, Draw, Place, A, SystemEvent], LayoutPlacementMeta[MU]]
)(
  val drawApi : SimpleDrawApi[MU, Draw],
  val placement : [Event] => (Axis, List[Place[Widget[Update, Draw, Place, Event, SystemEvent]]], MainAxisPlacementStrategy[MU], AdditionalAxisPlacementStrategy) => Place[List[(Widget[Update, Draw, Place, Event, SystemEvent], LayoutPlacementMeta[MU])]]
) extends HighLevelApi with LabelApi[TextStyle] with StatefulApi with LayoutApi[MU]:
  override type WidgetTask[+T] = WidgetTaskIn[T]
  override type Widget[+Event] = Place[me.katze.gui4s.widget.Widget[Update, Draw, Place, Event, SystemEvent]]
  ...
  private def linearLayout[Event](
                                  ...
                                  )(using Widget[Event] =:= Place[widget.Widget[Update, Draw, Place, Event, SystemEvent]]): Widget[Event] =
    linearLayoutLibrary.layout(children, placement[Event](axis, _, mainAxisStrategy, additionalAxisStrategy))
  end linearLayout
  
  
  private def addTaskResultCatcher[T](using RichTypeChecker[T])(name: String)(initial: Place[widget.Widget[Update, Draw, Place, T, SystemEvent]]) : Place[widget.Widget[Update, Draw, Place, T, SystemEvent]] =

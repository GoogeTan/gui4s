...
import me.katze.gui4s.widget.library.{LabelDraw, LayoutDraw, LiftEventReaction, TextPlacement}
...
trait Gui4sApp[
  ...
  final override def run(args: List[String]): IO[ExitCode] =
    for
      ...
      code <- drawApi(eventBus).use((windowSize, drawLoop, layoutDraw, labelDraw) =>
        given layoutDraw.type = layoutDraw
        given labelDraw.type = labelDraw

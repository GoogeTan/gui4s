 ...
 */
trait ForeignFunctionInterface[F[_]]:
  ...
  final def mapK[G[_]](f : F ~> G) : ForeignFunctionInterface[G] =
    new ForeignFunctionInterface[G]:
      override def delay[A](trunk: => A): G[A] = f(ForeignFunctionInterface.this.delay(trunk))

      override def blocking[A](trunk: => A): G[A] = f(ForeignFunctionInterface.this.blocking(trunk))

      override def interruptible[A](trunk: => A): G[A] = f(ForeignFunctionInterface.this.interruptible(trunk))

      override def interruptibleMany[A](trunk: => A): G[A] = f(ForeignFunctionInterface.this.interruptibleMany(trunk))
    end new
  end mapK
end ForeignFunctionInterface